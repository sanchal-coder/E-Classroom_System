...
<body>
    ...
    // Save results to the database only once
    ...
    try {
        ...
        // Check if the results are already stored for the user
        boolean resultsExist = false;
        String checkQuery = "SELECT * FROM result WHERE userId = ?";
        ...
        // Store the results if they don't exist for the user
        if (!resultsExist) {
            String insertQuery = "INSERT INTO result (userId, correctAnswers, attemptedAnswers, wrongAnswers) VALUES (?, ?, ?, ?)";
            stmt = conn.prepareStatement(insertQuery);
            stmt.setString(1, userId);
            stmt.setInt(2, correctAnswer);
            stmt.setInt(3, attempted);
            stmt.setInt(4, wrongAnswer);
            stmt.executeUpdate();
        }
